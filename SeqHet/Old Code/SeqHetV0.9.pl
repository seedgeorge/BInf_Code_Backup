...
# -----------------	------------------ # this loads the entry gene/variants list into a hash, duplicates are alerted to the user - in the case of a duplicate, only the first instance is recorded in the main hashBut I have some redrafts due in the beginning of November, so I have to get on that pretty sharpish.  
...
# print Dumper(\%rhash); # is handy if you just want to print the entire hash, but it can be a bit massive
